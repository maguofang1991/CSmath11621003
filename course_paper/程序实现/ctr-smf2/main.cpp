...
void print_usage_and_exit() {
  ...
  printf("      --lambda_f:       item vector regularizer, default 1\n");
  printf("\n");
  ...
  printf("      --how_trust_init: trust information about how user trusts each other\n");
  printf("      --is_trust_init:  trust information about weather user trusts each other or not\n");
...
int main(int argc, char* argv[]) {
  ...
  const char* const short_options = "hd:x:i:a:b:u:v:f:r:s:m:k:t:e:y:c:z:";
  const struct option long_options[] = {
    ...
    {"lambda_f",    required_argument, NULL, 'f'},
    ...
    {"how_trust_init", required_argument, NULL, 'c'},
    {"is_trust_init", required_argument, NULL, 'z'},
  ...
  char*  is_init_path = NULL;
  char*  how_init_path = NULL;
  ...
  while(true) {
    ...
    switch(cc) {
      ...
      case 'z':
    	...
    	break;
      case 'c':
    	how_init_path = optarg;
    	break;
      case 'f':
    	lambda_f = atof(optarg);
        break;
	case 'l':
	test_path = optarg;
	break;
  ...
  ctr_param.set(a, b, lambda_u, lambda_v, lambda_f, random_seed, max_iter, save_lag, theta_opt, ctr_run);
  ...
  ctr->read_info(is_init_path, how_init_path);
  ...
  if (!file_exists(test_path) ) {
    	printf("test file %s doesn't exist! quit ...\n", test_path);
    	exit(-1);
   }
  else{
	  printf("test file: %s\n",test_path);
	  ...
	  c_data* test_users = new c_data();
	  test_users->read_data(test_path);

	  int num_test_users = (int)test_users->m_vec_data.size();

	  //cal mae rsme
	  c_eval* eval = new c_eval();
	  eval->set_parameters(directory,num_factors,num_users,num_items);
	  // create model instance
	  ctr->learn_map_estimate(users, items, c, &ctr_param, directory,test_users,num_test_users,eval);
