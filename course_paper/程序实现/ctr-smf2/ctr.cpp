...
c_ctr::~c_ctr() {
  ...
  if(m_is_trust!=NULL)gsl_matrix_free(m_is_trust);
  if(m_how_trust!=NULL)gsl_matrix_free(m_how_trust);

}

void c_ctr::read_info( const char* is_trust__path,
        const char* how_trust_path){
...
void c_ctr::learn_map_estimate(const c_data* users, const c_data* items, 
                               ...
                               const char* directory,const c_data* test_users,int num_test_users,c_eval* eval) {
        ...
        //********************calc lambda_fUT' + VCiRi******************
		  ...
		  gsl_vector_memcpy(vector_tmp, &how_trust_row.vector);
       	 ...
       	 gsl_blas_dgemv(CblasTrans,
       			  param->lambda_f, m_U, vector_tmp, 0.0, YY);
